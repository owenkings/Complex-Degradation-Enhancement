...
### 1. 数据加载验证
在开始训练前，验证 CUB-C 数据集加载是否正常：
```bash
python task1/debug_dataloader.py
```
...
```
- **输出**：训练日志 (`task1/logs/train_log.csv`) 和模型权重 (`*.pth`) 保存在 `task1/checkpoints_task1_restormer`。

### 3. 结果绘图 (Step 1)
训练完成后，绘制 Loss 曲线及验证集 PSNR/SSIM 变化图。
```bash
python task1/plot_task1_loss.py \
  --log-csv task1/logs/train_log.csv \
  --out-dir task1/plots
```
...
python task1/eval_task1_imagenetc_vgg16.py \
  ...
  --out-json task1/logs/task1_imagenetc_results.json \
  --severity 1 2 3 4 5 \
  --corruptions fog snow brightness contrast motion_blur \
  --batch-size 32 \
  --num-workers 8
```
- **参数说明**：
  - `--corruptions`: 指定测试的降质类型 (如 `fog,motion_blur` 或 `all`)。
  - `--severities`: 指定测试的强度等级 (通常为 `1,2,3,4,5`)。
- **输出**：
...
### 1. 环境验证
检查 Mamba 模块安装及 Python 路径：
```bash
python task2/find_module_name.py
python task2/verify_path.py
```

### 2. 训练 Feature Enhancer
...
python task2/eval_task2.py \
  --data-root data/ImageNet-Val-C \
  --synset-mapping data/ImageNet-Val-C/synset_mapping.txt \
  --ckpt task2/checkpoints/mamba_enhancer_best.pth \
  --out-json task2/logs/task2_imagenetc_results.json \
  --severity 1 2 3 4 5 \
  --corruptions fog snow brightness contrast motion_blur \
  --batch-size 64 \
  --num-workers 8
